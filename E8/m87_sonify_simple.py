#!/usr/bin/env python3
"""
M87* E8 Fractal Sonification (corrected clean version)
"""
...
OUT_WAV = SCRIPT_DIR / "M87_E8_Fractal_Sonification.wav"
...
def fitness(a):
    return 0.1 + 0.32 * np.exp(-((np.abs(a) - 0.55) ** 2) / 0.9) + 0.1 * (np.abs(a) / 0.97)
...
def main():
    ...
    for i, (a, mu, sg) in enumerate(zip(SPINS, MUS, SIGS)):
        ...
        env = np.ones(len(ts))
        atk = max(2, int(0.06 * SR))
        rel = max(2, int(0.1 * SR))
    ...
    fo = fi
    L[:fi] *= np.linspace(0, 1, fi) ** 2
    R[:fi] *= np.linspace(0, 1, fi) ** 2
    L[-fo:] *= np.linspace(1, 0, fo) ** 2
    R[-fo:] *= np.linspace(1, 0, fo) ** 2

    out = np.stack([L, R], axis=1).astype(np.float32)
    wavfile.write(str(OUT_WAV), SR, np.clip(out * 32767, -32768, 32767).astype(np.int16))
    print(f"Done! Wrote {OUT_WAV}")
    print(f"  {DUR}s @ {SR}Hz stereo")
